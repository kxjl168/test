...
public class WsFrame {
	...
	public static byte[] buildFrame(byte[] sendBytes, OPCODE type) {
		...
		if (length <= 125) {
			...
			long blength = sendBytes.length;

			buf = new byte[length + 2 + 8];

			byte b2 = 0x0;
			b2 |= 127; // fin =1 buf[0] = b1; buf[1] = b2;
			buf[0] = b1;
			buf[1] = b2;

			buf[2] = (byte) ((blength >> 56) & 0xFF);//
			buf[3] = (byte) ((blength >> 48) & 0xFF);//
			buf[4] = (byte) ((blength >> 40) & 0xFF); //
			buf[5] = (byte) ((blength >> 32) & 0xFF); //
			buf[6] = (byte) ((blength >> 24) & 0xFF); //
			buf[7] = (byte) ((blength >> 16) & 0xFF); //
			buf[8] = (byte) ((blength >> 8) & 0xFF); //
			buf[9] = (byte) ((blength) & 0xFF); //
			...
			try {
				...
				if (opcode == OPCODE.TEXT || opcode == OPCODE.BIANRY) {
...

					{
						 ...
						 * defalterByte = Arrays.copyOf(alldata,
						 * alldata.length);
						 * 
						 * byte[] result = new byte[2014]; Inflater inf = new
						 * Inflater(); inf.setInput(defalterByte); int infLen =
						 * inf.inflate(result); inf.end(); String strOgr = new
						 * String(result, "utf-8"); //
						 * System.out.println("str ogr "+strOgr); // return
						 * Arrays.copyOf(result, infLen);
						 */}
				}
